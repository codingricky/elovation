class Api::PlayerSlackAttachment
  def self.create_slack_attachment_from(player)
    ...
    attachment['fields'] = [create_field('wins', player.total_wins(Game.default), true),
                            ...
                            create_field('wins broken up by days of the week', format_wins_by_day_of_the_week(player.wins_by_day_of_the_week), false),
                            create_field('last 10 results', last_10_results, false)]
    return attachment
  end

  def self.format_wins_by_day_of_the_week(wins_by_days)
    days_of_the_week = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']
    days_of_the_week.collect do |day|
      "#{day} - #{ActionController::Base.helpers.number_to_percentage(wins_by_days[day])}"
    end.join("\n")
